...
const Header = () => {
  ...
  // Removed local notifications duplication; rely on persisted zustand store

  // Get the Notifications from the zustand store
  ...
  useEffect(() => {
    ...
    socket.on("notification", (data) => {
      ...
      const formatNotification = {
        id: Date.now(),
        type: data?.type === 'student_enrolled' ? 'success' : 'info',
        title: data?.message || 'Notification',
        message: data?.data?.courseTitle || data?.data?.courseId || 'Action recorded',
        time: new Date().toLocaleTimeString(),
      };
      addNotification(formatNotification);
    });
  ...
  //     setTimeout(() => {
  //       addNotification({
  //         // id auto-generated in store if omitted
  //         type: "success",
                ...
                {isAuth && username ? (
                  <>
                    ...
                    <div className="relative">
                      ...
                      {showUserMenu && (
                        <div
                          className={`user-menu-dropdown absolute right-0 mt-2 w-64 ${
                            isDark
                              ? "bg-gray-800 border-gray-700"
                              : "bg-white border-gray-200"
                          } rounded-xl shadow-xl border z-50 animate-in slide-in-from-top-2 duration-300`}
