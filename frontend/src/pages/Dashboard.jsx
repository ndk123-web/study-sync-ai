...
const Dashboard = () => {
    ...
    const fetchTrendAnalysis = async () => {
      ...
      try {
        ...
        if (apiResponse?.data?.availableYears?.length > 0) {
          setSelectedYear(
            apiResponse.data.availableYears[
              apiResponse.data.availableYears.length - 1
            ]
          );
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const fetchTopicsWiseProgress = async () => {
        ...
        setCategoryDistribution(apiResponse?.data?.categoryDistribution || []);
      };
  ...
  const ResponsiveChartBox = ({ children, min = 220, vh = 35, max = 420 }) => (
    <div
      style={{ width: "100%", height: `clamp(${min}px, ${vh}vh, ${max}px)` }}
    >
  ...
  const userNotes = [
    {
      id: 1,
      title: "React Hooks",
      content: "useState, useEffect, and custom hooks overview...",
      tags: ["react", "hooks"],
      date: "Aug 10, 2025",
    },
    {
      id: 2,
      title: "SQL Joins",
      content: "INNER, LEFT, RIGHT joins explained with examples...",
      tags: ["sql", "db"],
      date: "Aug 9, 2025",
    },
    {
  ...
  const TrendAnalysisCard = () => {
    ...
    return (
      <div
        ...
        } border rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300`}
      >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-8">
            {statsCards.map((stat, index) => (
              ...
              >
                ...
                <div
                  className={`${stat.bgColor} w-10 h-10 lg:w-12 lg:h-12 rounded-lg flex items-center justify-center mb-3 transform hover:rotate-12 transition-transform relative z-10`}
                >
                  <div
              ...
              {quickActions.map((action, index) => (
                ...
                >
                  ...
                  <div className="relative z-10">
                    <div
                      className={`${action.color} w-12 h-12 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-lg bg-gradient-to-r text-white text-2xl`}
                    >
