...
app.use(
  cors({
    origin: ["http://localhost:5173", "https://studysync.ndkdev.me"],
...
async function uploadPDFToCloudinary(pdfBuffer, studentName, courseName) {
  try {
    ...
    const cleanStudentName = (studentName || "student").replace(
      /[^a-zA-Z0-9]/g,
      "_"
    );
    const cleanCourseName = (courseName || "course").replace(
      /[^a-zA-Z0-9]/g,
      "_"
    );
...
app.get("/get-certificate/:publicId", async (req, res) => {
  try {
    ...
    const result = await cloudinary.api.resource(
      `study-sync-certificates/${publicId}`,
      {
        resource_type: "raw",
      }
    );
...
app.listen(PORT, () => {
  ...
  console.log(
    `📄 Generate Certificate: http://localhost:${PORT}/generate-certificate?name=John&course=React`
  );
  console.log(
    `📋 List Certificates: http://localhost:${PORT}/list-certificates`
  );
});
